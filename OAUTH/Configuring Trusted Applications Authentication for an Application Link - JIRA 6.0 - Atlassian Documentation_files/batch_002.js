...
Confluence.Templates.User.logo = function(opt_data, opt_sb) {
  ...
  if (opt_data.profilePictureInfo['default'] && opt_data.username == opt_data.currentUsername) {
    output.append('<a ', (opt_data.canView) ? ' ' + ((! opt_data.disableUserHover) ? 'class="userLogoLink"' : '') + ' data-username="' + soy.$$escapeHtml(opt_data.username) + '"' : '', ' href="', soy.$$escapeHtml(""), '/users/editmyprofilepicture.action" title="', soy.$$escapeHtml("Add a picture of yourself"), '"><img class="userLogo logo defaultLogo" src="', soy.$$escapeHtml("/s/en_GB/5523/205577d01f4fb4efed99ead6d83b28ba8b59d647.27/_"), '/images/icons/profilepics/add_profile_pic.png" alt="', soy.$$escapeHtml("User icon"), ': ', soy.$$escapeHtml("Add a picture of yourself"), '"></a>');
  } else {
    if (opt_data.profilePictureInfo.anonymous) {
      output.append('<img class="userLogo logo anonymous" src="', soy.$$escapeHtml("/s/en_GB/5523/205577d01f4fb4efed99ead6d83b28ba8b59d647.27/_"), '/images/icons/profilepics/anonymous.png" alt="', soy.$$escapeHtml("User icon"), ': ', soy.$$escapeHtml("Anonymous"), '" title="', soy.$$escapeHtml("Anonymous"), '">');
      ...
      output.append('<img class="userLogo logo anonymous" src="', soy.$$escapeHtml("/s/en_GB/5523/205577d01f4fb4efed99ead6d83b28ba8b59d647.27/_"), '/images/icons/profilepics/anonymous.png" alt="', soy.$$escapeHtml("User icon"), ': ', soy.$$escapeHtml(opt_data.username), '" title="', soy.$$escapeHtml(opt_data.username), '">');
    }
  }
  return opt_sb ? '' : output.toString();
};
...
try {
/* module-key = 'com.atlassian.analytics.analytics-client:js-events', location = 'js/jquery-json-2.3.js' */
...
var $ajax=AJS.$.ajax;(function(f){var h="atlassian-analytics";var e=typeof AJS.contextPath=="function"?AJS.contextPath():typeof AJS.Confluence!="undefined"?AJS.Confluence.getContextPath():window.contextPath!=null?window.contextPath:"";var d=null;var l=null;var c=null;var a="xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(q){var p=Math.random()*16|0,o=q=="x"?p:(p&3|8);return o.toString(16)});var b=function(){var o="unknown";if(document.body.id=="jira"){o="jira"}else{if(document.body.id=="com-atlassian-confluence"){o="confluence"}}l=h+"."+o;c=l+".lock"};var n=function(){if(store.get(c)){return false}store.set(c,a);return(store.get(c)===a)};var j=function(){store.set(c,null)};var g=function(){var p=[],r,s,o,q;if(AJS.EventQueue.length==0){return}p=store.get(l)||p;for(o=0,q=AJS.EventQueue.length;o<q;++o){s=AJS.EventQueue[o];if(s.name){r={name:s.name,properties:s.properties,time:s.time||0};p.push(r)}}AJS.EventQueue.length=0;store.set(l,p)};var k=function(){var p;if(!n()){return}g();p=store.get(l);if(p&&p.length){store.remove(l);j();var q=new Date().getTime();for(var o=0;o<p.length;o++){if(p[o].time>0){p[o].timeDelta=p[o].time-q}else{p[o].timeDelta=o-p.length}delete p[o].time}d=$ajax({type:"POST",url:e+"/rest/analytics/1.0/publish/bulk",data:f.toJSON(p),contentType:"application/json",dataType:"json"});d.fail(function(){AJS.EventQueue.concat(p);g()})}else{j()}};var m=function(){if(d&&!(d.isResolved()||d.isRejected())){d.abort()}};AJS.EventQueue=AJS.EventQueue||[];var i=Array.prototype.push;AJS.EventQueue.push=function(o){o.time=new Date().getTime();i.call(AJS.EventQueue,o)};AJS.toInit(function(){b();k();setInterval(k,5000)});f(window).unload(function(){m();g()});AJS.bind("analytics",function(o,p){AJS.EventQueue.push({name:p.name,properties:p.data})});AJS.bind("analyticsEvent",function(o,p){AJS.EventQueue.push({name:p.name,properties:p.data})})}(AJS.$));
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
