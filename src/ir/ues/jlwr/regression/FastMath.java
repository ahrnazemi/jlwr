 ...
 * <p>
 * FastMath is a drop-in replacement for both Math and StrictMath. This means
 * that for any method in Math (say {@code Math.sin(x)} or {@code Math.cbrt(y)}
 * ), user can directly change the class and use the methods as is (using
 ...
 * native code present in many JVMs today and use of large tables. The larger
 * tables are lazily initialised on first use, so that the setup time does not
 * penalise methods that don't need them.
 * </p>
 * <p>
 * Note that FastMath is extensively used inside Apache Commons Math, so by
 * calling some algorithms, the overhead when the the tables need to be
 * intialised will occur regardless of the end-user calling FastMath methods
 * directly or not. Performance figures for a specific JVM and hardware can be
 * evaluated by running the FastMathTestPerformance tests in the test directory
 ...
 * href
 * ="http://en.wikipedia.org/wiki/Rounding#The_table-maker.27s_dilemma">Table
 * Maker's Dilemma</a>).
 * </p>
 * <p>
...
public class FastMath {
	 ...
	 * 
	 * Returns {@code NaN} if either argument is negative. If {@code base} is 0
	 * and {@code x} is positive, 0 is returned. If {@code base} is positive and
	 * {@code x} is 0, {@code Double.NEGATIVE_INFINITY} is returned. If both
	 * arguments are 0, the result is {@code NaN}.
	 * 
	 * @param base
	...
	private static void reducePayneHanek(double x, double result[]) {
		...
		double tmpB = (((prod2A & 0xfffL) << 40) + (prod2B >>> 24))
				/ TWO_POWER_52 / TWO_POWER_52; // Low bits
	...
	public static double scalb(final double d, final int n) {
		...
		if (n < 0) {
			...
			if (scaledExponent > 0) {
				...
				// bits
				final long mostSignificantLostBit = mantissa
						& (1L << (-scaledExponent));
	...
	public static float scalb(final float f, final int n) {
		...
		if (n < 0) {
			...
			if (scaledExponent > 0) {
				...
				// scales down complete mantissa, hence losing least significant
				// bits
				final int mostSignificantLostBit = mantissa
						& (1 << (-scaledExponent));
				mantissa = mantissa >>> (1 - scaledExponent);
				if (mostSignificantLostBit != 0) {
					// we need to add 1 bit to round up the result
