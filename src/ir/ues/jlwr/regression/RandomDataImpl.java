 ...
 * to use a {@link Well19937c} generator. To plug in a different implementation,
 ...
 * <code>RandomGenerator</code> to the constructor, however, this generator is
 * not reseeded by the constructor nor is it reseeded on first use.</li>
 * <li>
...
public class RandomDataImpl implements RandomData, Serializable {
	...
	/**
	 * Used when generating Exponential samples. Table containing the constants
	 * q_i = sum_{j=1}^i (ln 2)^j/j! = ln 2 + (ln 2)^2/2 + ... + (ln 2)^i/i!
	 * until the largest representable fraction below 1 is exceeded.
	 * 
	 * Note that 1 = 2 - 1 = exp(ln 2) - 1 = sum_{n=1}^infty (ln 2)^n / n! thus
	 * q_i -> 1 as i -> infty, so the higher i, the closer to one we get (the
	 * series is not alternating).
	 * 
	...
	static {
		/**
		 * Filling EXPONENTIAL_SA_QI table. Note that we don't want qi = 0 in
		 * the table.
		 */
		 ...
		 * together with Precision.EPSILON to generate the following code (a
		 * priori, we know that there will be 16 elements, but instead of
		 * hardcoding that, this is prettier):
		 */
	 ...
	 * {@code System.currentTimeMillis() + System.identityHashCode(this))}. The
	 * generator is initialized and seeded on first use.
	 * </p>
	 */
	 ...
	 * 
	 * @param rand
	 *            the source of (non-secure) random data (may be null, resulting
	 ...
	 * <ul>
	 * <li>For small means, uses simulation of a Poisson process using Uniform
	 * deviates, as described <a
	 * href="http://irmi.epfl.ch/cmos/Pmmi/interactive/rng7.htm"> here.</a> The
	 * Poisson process (and hence value returned) is bounded by 1000 * mean.</li>
	 * 
	 * <li>For large means, uses the rejection algorithm described in <br/>
	 * Devroye, Luc. (1981).<i>The Computer Generation of Poisson Random
	 * Variables</i> <strong>Computing</strong> vol. 26 pp. 197-207.</li>
	 * </ul>
	 * </p>
	 */
	public long nextPoisson(double mean) {
		...
		if (mean < pivot) {
			...
			final double logLambdaFactorial = ArithmeticUtils
			...
			for (;;) {
				...
				if (v < y * logLambda
						- ArithmeticUtils.factorialLog((int) (y + lambda))
						+ logLambdaFactorial) {
					y = lambda + y;
					break;
				}
	...
	public double nextGaussian(double mu, double sigma) {
		...
		if (sigma <= 0) {
			throw new NotStrictlyPositiveException(
					LocalizedFormats.STANDARD_DEVIATION, sigma);
		}
	 ...
	 * from p. 876 in: [1]: Ahrens, J. H. and Dieter, U. (1972). Computer
	 * methods for sampling from the exponential and normal distributions.
	 ...
	 *             if one of the bounds is infinite or {@code NaN} or either
	 ...
	 * This is necessary to provide a symmetric output interval (both endpoints
	 * excluded).
	 * </p>
	 * 
	 * @throws MathIllegalArgumentException
	 *             if one of the bounds is infinite or {@code NaN}
	 * @since 3.0
	 */
	...
	/**
	 * Generates a random value from the {@link BetaDistribution Beta
	 * Distribution}. This implementation uses
	 * {@link #nextInversionDeviate(RealDistribution) inversion} to generate
	 * random values.
	 * 
	 * @param alpha
	...
	/**
	 * Generates a random value from the {@link BinomialDistribution Binomial
	 * Distribution}. This implementation uses
	 * {@link #nextInversionDeviate(RealDistribution) inversion} to generate
	 * random values.
	 * 
	...
	public int nextBinomial(int numberOfTrials, double probabilityOfSuccess) {
		return nextInversionDeviate(new BinomialDistribution(numberOfTrials,
				probabilityOfSuccess));
	}

	/**
	 * Generates a random value from the {@link CauchyDistribution Cauchy
	 * Distribution}. This implementation uses
	 * {@link #nextInversionDeviate(RealDistribution) inversion} to generate
	 * random values.
	 * 
	...
	/**
	 * Generates a random value from the {@link ChiSquaredDistribution ChiSquare
	 * Distribution}. This implementation uses
	 * {@link #nextInversionDeviate(RealDistribution) inversion} to generate
	 * random values.
	 * 
	 ...
	 * This implementation uses {@link #nextInversionDeviate(RealDistribution)
	 ...
	 * @return random value sampled from the F(numeratorDf, denominatorDf)
	 *         distribution
	 * @since 2.2
	 */
	public double nextF(double numeratorDf, double denominatorDf) {
		return nextInversionDeviate(new FDistribution(numeratorDf,
				denominatorDf));
	}
	 ...
	 * {@link org.apache.commons.math3.distribution.GammaDistribution Gamma
	 * Distribution}.
	 * </p>
	 * 
	 * <p>
	...
	/**
	 * Generates a random value from the {@link HypergeometricDistribution
	 * Hypergeometric Distribution}. This implementation uses
	 * {@link #nextInversionDeviate(IntegerDistribution) inversion} to generate
	 * random values.
	 * 
	 * @param populationSize
	 *            the population size of the Hypergeometric distribution
	 * @param numberOfSuccesses
	 *            number of successes in the population of the Hypergeometric
	 *            distribution
	 * @param sampleSize
	 *            the sample size of the Hypergeometric distribution
	 * @return random value sampled from the Hypergeometric(numberOfSuccesses,
	 *         sampleSize) distribution
	 * @since 2.2
	 */
	public int nextHypergeometric(int populationSize, int numberOfSuccesses,
			int sampleSize) {
		return nextInversionDeviate(new HypergeometricDistribution(
				populationSize, numberOfSuccesses, sampleSize));
	}

	/**
	 * Generates a random value from the {@link PascalDistribution Pascal
	 * Distribution}. This implementation uses
	 * {@link #nextInversionDeviate(IntegerDistribution) inversion} to generate
	 * random values.
	 * 
	...
	/**
	 * Generates a random value from the {@link WeibullDistribution Weibull
	 * Distribution}. This implementation uses
	 * {@link #nextInversionDeviate(RealDistribution) inversion} to generate
	 * random values.
	 * 
	...
	/**
	 * Generates a random value from the {@link ZipfDistribution Zipf
	 * Distribution}. This implementation uses
	 * {@link #nextInversionDeviate(IntegerDistribution) inversion} to generate
	 * random values.
	 * 
	 ...
	 * @return random value sampled from the Zipf(numberOfElements, exponent)
	 *         distribution
	 * @since 2.2
	 */
	public int nextZipf(int numberOfElements, double exponent) {
		return nextInversionDeviate(new ZipfDistribution(numberOfElements,
				exponent));
	}
	 ...
	 * <p>
	 * Creates and initializes a default generator if null. Uses a
	 * {@link Well19937c} generator with
	 * {@code System.currentTimeMillis() + System.identityHashCode(this))} as
	 * the default seed.
	 * </p>
	 * 
	 ...
	 * {@code System.currentTimeMillis() + System.identityHashCode(this)} as the
	 * default seed.
	 * </p>
	 * 
	...
	public int[] nextPermutation(int n, int k) {
		if (k > n) {
			throw new NumberIsTooLargeException(
					LocalizedFormats.PERMUTATION_EXCEEDS_N, k, n, true);
		}
		if (k <= 0) {
			throw new NotStrictlyPositiveException(
					LocalizedFormats.PERMUTATION_SIZE, k);
		}
	...
	public Object[] nextSample(Collection<?> c, int k) {
		...
		if (k > len) {
			throw new NumberIsTooLargeException(
					LocalizedFormats.SAMPLE_SIZE_EXCEEDS_COLLECTION_SIZE, k,
					len, true);
		}
		if (k <= 0) {
			throw new NotStrictlyPositiveException(
					LocalizedFormats.NUMBER_OF_SAMPLES, k);
		}
	 ...
	 * href="http://en.wikipedia.org/wiki/Inverse_transform_sampling"> inversion
	 * method.</a>
	 * 
	 * @param distribution
