...
import ir.ues.jlwr.regression.exception.NotPositiveException;
 ...
 * @version $Id: HypergeometricDistribution.java 1244107 2012-02-14 16:17:55Z
 *          erans $
 */
public class HypergeometricDistribution extends AbstractIntegerDistribution {
	 ...
	 *             if {@code numberOfSuccesses > populationSize}, or
	 *             {@code sampleSize > populationSize}.
	 */
	public HypergeometricDistribution(int populationSize,
			int numberOfSuccesses, int sampleSize) throws NotPositiveException,
			NotStrictlyPositiveException, NumberIsTooLargeException {
		if (populationSize <= 0) {
	...
	public double probability(int x) {
		...
		if (x < domain[0] || x > domain[1]) {
			...
			double p2 = SaddlePointExpansion.logBinomialProbability(sampleSize
					- x, populationSize - numberOfSuccesses, p, q);
	 ...
	 * 
	 * @param x
	 *            Value at which the PMF is evaluated.
	 * @param n
	 *            the population size.
	 * @param m
	 *            number of successes in the population.
	 ...
	 * {@code P(x0 <= X <= x1)}. This probability is computed by summing the
	 * point probabilities for the values {@code x0, x0 + 1, x0 + 2, ..., x1},
	 * in the order directed by {@code dx}.
	 * 
	 * @param x0
