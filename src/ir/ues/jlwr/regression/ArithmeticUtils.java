...
public final class ArithmeticUtils {
	...
	public static double binomialCoefficientLog(final int n, final int k) {
		...
		/*
		 * For values small enough to do exact integer computation, return the
		 * log of the exact value
		 */
		if (n < 67) {
	 ...
	 * {@code double}. The result should be small enough to fit into a
	 * {@code double}: The largest {@code n} for which
	 * {@code n! < Double.MAX_VALUE} is 170. If the computed value exceeds
	...
	private static void checkBinomial(final int n, final int k) {
		if (n < k) {
			throw new NumberIsTooLargeException(
					LocalizedFormats.BINOMIAL_INVALID_PARAMETERS_ORDER, k, n,
					true);
		}
		if (n < 0) {
			throw new NotPositiveException(
